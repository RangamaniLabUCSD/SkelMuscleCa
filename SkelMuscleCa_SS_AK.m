function yInf = SkelMuscleCa_SS_AK(lowATP, selParam)
...
selIdx = [7,16,19,21,30,50]; % J_NaK, g_Cl, g_NCX, g_Na, g_K, G_K 
...
function dydt = f(y, p, lowATP)
    ...
    % Geometry 
    vol_Fiber = 2 * PI * (R_fiber^2) * L_fiber; 
	...
	LumpedJ_SERCA = (602.214179 .* SA_SRM .* nu_SERCA .* c_i ./ (K_SERCA + c_i));
	...
	F_PM = ( - (I_DHPR .* SA_PM) - (I_PMCA .* SA_PM) - (I_K_DR .* SA_PM) - (I_K_IR .* SA_PM) - (I_NKX_K .* SA_PM) - (I_Na .* SA_PM) - (I_Cl .* SA_PM) - (I_NCX_C .* SA_PM) - (I_NKX_N .* SA_PM) - (I_NCX_N .* SA_PM) - (I_SOCE .* SA_PM) - (I_CaLeak_PM .* SA_PM));
	L_RyR = (1000.0 ./ 0.002);
	...
	LumpedJ_CaLeak_SR = (602.214179 .* SA_SRM .* nu_leakSR .* (c_SR - c_i));
	...
	dydt = [
		...
		f_SR*( - (KFlux_SRM_SR .* LumpedJ_RyR .* UnitFactor_uM_um3_molecules_neg_1 ./ SA_SRM) + (KFlux_SRM_SR .* LumpedJ_SERCA .* UnitFactor_uM_um3_molecules_neg_1 ./ SA_SRM) - (LumpedJ_CaLeak_SR .* KFlux_SRM_SR .* UnitFactor_uM_um3_molecules_neg_1 ./ SA_SRM));    % rate for c_SR
		J_r5;    % rate for h_K
		J_r0;    % rate for w_RyR
		J_r7;    % rate for w_DHPR
		((1000 .* ((I_CaLeak_PM .* SA_PM) + (I_Cl .* SA_PM) + (I_DHPR .* SA_PM) + (I_K_DR .* SA_PM) + (I_K_IR .* SA_PM) + (I_NCX_C .* SA_PM) + (I_NCX_N .* SA_PM) + (I_NKX_K .* SA_PM) + (I_NKX_N .* SA_PM) + (I_Na .* SA_PM) + (I_PMCA .* SA_PM) + (SA_PM .* I_SOCE) + I_PM)) ./ device_PM.Capacitance);    % rate for Voltage_PM
		((KFlux_PM_cyto .* J_Na) - (KFlux_PM_cyto .* J_NKX_N) + (KFlux_PM_cyto .* J_NCX_N));    % rate for Na_i
		0;%(J_Cl .* KFlux_PM_cyto);    % rate for Cl_i
		f_i*((KFlux_SRM_cyto .* LumpedJ_RyR .* UnitFactor_uM_um3_molecules_neg_1 ./ SA_SRM) + (J_DHPR .* KFlux_PM_cyto) - (KFlux_PM_cyto .* J_PMCA) - (KFlux_SRM_cyto .* LumpedJ_SERCA .* UnitFactor_uM_um3_molecules_neg_1 ./ SA_SRM) + (LumpedJ_CaLeak_SR .* KFlux_SRM_cyto .* UnitFactor_uM_um3_molecules_neg_1 ./ SA_SRM) - (KFlux_PM_cyto .* J_NCX_C)  + (KFlux_PM_cyto .* J_SOCE) + (J_CaLeak_PM .* KFlux_PM_cyto));    % rate for c_i
		J_r4;    % rate for n
		J_r2;    % rate for m
		J_r1;    % rate for h
		J_r3;    % rate for S
