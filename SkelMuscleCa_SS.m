function [Time,Y,Rate,nRate,currtime] = SkelMuscleCa_SS(tSpan,freq, lowATP, yinit, p,StartTimer)
...
options = odeset('RelTol',1e-8,'MaxStep',.001,'NonNegative',[1:5,7:17]);%,'OutputFcn',@odeplot);
[Time,Y] = ode15s(@f,timeSpan,yinit,options,p,freq,lowATP); %pass extra arguments at the end

Rate = zeros(size(Y));
nRate = zeros(size(Y));

for i = 1 : length(Time)
    Rate(i,:) = f(Time(i),Y(i,:),p,freq,lowATP);
    nRate(i,:) = Rate(i,:) ./ Y(i,:);
end  
...
% ode rate
    ...
    %R_threshold = false;
        ...
        %while ~R_threshold
            ...
            %R = abs(dydt([1:10,12:17]));
            %R_threshold = all(R < threshold);
        end    
    end
%end
