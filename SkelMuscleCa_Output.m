...
xlabel('Time (s)', 'Fontsize',16)
ylabel('[Ca^{2+}]_{SR} (μM)','Fontsize',16,'FontSmoothing','on')
legend('Control','SOCE Blocked','Fontsize',14,'Edgecolor','none', 'Color','none')
set(axes7,'FontSize',16,'Box','off','FontSmoothing','on')
set(plot7,"Renderer","painters");
%% -------------------------------------------------------------------------------
plot8 = figure; % DHPR flux Plot
axes8 = axes('Parent', plot8);
semilogy(Time2,fluxes2(:,4),'LineWidth',2,'Color',[0.49,0.18,0.56]);
hold on
semilogy(Time_noSOCE4,fluxes_noSOCE4(:,4),'LineWidth',2,'Color',[0.10,0.85,0.83],'LineStyle','--')
hold off
xlim([0 0.1])
xlabel('Time (s)', 'Fontsize',16)
ylabel('Flux (μM/s)','Fontsize',16)
...
%% -------------------------------------------------------------------------------
Fig3a(Time_noSOCE4,Y_noSOCE4(:,5),Y_noSOCE4(:,8))                            % V_SL and [Ca^2+]_myo over time

%% Wei-LaPierre Stimulus - 50Hz 0.5s every 2.5s Expt 0 and 1
Fig3a(Time_noSOCE0,Y_noSOCE0(:,5),Y_noSOCE0(:,8))                            % V_SL and [Ca^2+]_myo over time
%% 
Fig3a(Time1,Y1(:,5),Y1(:,8))                            % V_SL and [Ca^2+]_myo over time

%% Stimulus Plot 
plot9 = figure;
axes9  = axes('Parent',plot9);
plot(Time_noSOCE0,currents_noSOCE0(:,13),"LineWidth",2)
ylabel('I_{SL} (pA)','Fontsize',16)
set(axes9,'FontSize',16,'Box', 'off','FontSmoothing','on');
set(plot9,"Renderer","painters");
%% Peak Ca2+ per stimulus plot ---------------------------------------------------
t = 0 : 2.5 : max(Time1);
MaxCa = zeros(length(t),1);
MaxCa_noSOCE = zeros(length(t),1);
for j = 1 : (length(t) - 1)
    t_index = (Time1 > t(j)) & (Time1 < t(j+1));
    timepts = Time1(t_index);
    MaxCa(j) = max(Y1(t_index,8));
    % When SOCE channel is blocked i.e. g_SOCE = 0
    t_index_noSOCE = (Time_noSOCE0 > t(j)) & (Time_noSOCE0 < t(j+1));
    timepts_noSOCE = Time_noSOCE0(t_index_noSOCE);
    MaxCa_noSOCE(j) = max(Y_noSOCE0(t_index_noSOCE,8));
end

plot10 = figure; 
axes10  = axes('Parent',plot10);
scatter(1:length(MaxCa)-1,MaxCa(1:end-1),'filled', 'o','MarkerFaceColor',[0.635294117647059 0.0784313725490196 0.184313725490196]) 
hold on
scatter(1:length(MaxCa_noSOCE)-1,MaxCa_noSOCE(1:end-1),'filled','o','MarkerFaceColor',[0.10,0.85,0.83])
hold off
