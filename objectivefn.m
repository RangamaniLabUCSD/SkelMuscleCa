% function yinf = LM_SS(yinit, expt,p)
% initialGuess = yinit';
% % Set LM algorithm parameters.
% options = optimoptions('lsqnonlin', 'Algorithm', 'levenberg-marquardt', 'Display', 'iter', 'MaxIter', 1000);
% lb = 0.5 * initialGuess;
% ub = 2 * initialGuess;
% lb(6,1) = -100;
% ub(6,1) = -60;
...
% if n < 3 || n > 4
%     % try
%     [yinf] = lsqnonlin(@(initialGuess) objectivefn(initialGuess,n,param),initialGuess, lb, ub, options);
% 
% end

% Define your objective function that computes the sum of squared errors (MSE).
function mse = objectivefn(y,expt,p)
    % Replace this with your model equations that produce output.
    % Compute the difference between the computed output and experimental data.
    % Return the sum of squared errors as the objective value.
    % Example:
    % computedOutput = YourModelEquations(params);
    % mse = sum((computedOutput - experimentalData).^2);
...
%% State Variables
   ...
   %[K_t K_V], [B_t B_V] , [M_t M_V], [MJ_t MJ_V]};
   Ca_o_exp = [1000, 1000, 2000, 2000,2000, 2500, 1800, 5000, 2000];              %mM
   Na_o_exp = [138100, 138100, 150000, 150000, 150000, 143800, 118240, 140000, 151000] ; %mM
   K_o_exp = [3900, 3900, 2000, 2000, 2000, 5000, 5330, 4000, 5000];               %mM
   Cl_o_exp = [143700, 143700, 158000, 158000,158000, 124000, 126170, 157000, 146000];  %mM
   Temp = [(273+22),(273+22),(273+20),(273+22), (273+22),(273+ 26),(273+22),(273+22),(273+22)]; %K

    % Constants
    c_EC_init_uM = Ca_o_exp(expt); %1300.0;
    Cl_EC_init_uM = Cl_o_exp(expt); %128000.0;
    K_EC_init_uM = K_o_exp(expt); %4000.0;
    Na_EC_init_uM = Na_o_exp(expt); %147000.0;
    ...
    if currtime> 600
         mse = 1/0;
         fprintf('Too long to execute LM')
         return
    end
    ...
    Nf = [1;1000;1;1;1;-90;10000;5000;0.1;1;1;1;1;100000;1500;1500;1]; %Normalization factor
