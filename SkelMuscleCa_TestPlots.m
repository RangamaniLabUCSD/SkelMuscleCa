...
load PSOpenaltyNoBib_Allv2_11-May-2025.mat pSol
pSol([12,95]) = 1;
% pSol(106) = 1;
...
pPSO0 = pPSO;
pPSO0(95) = 0; % no SOCE
[TimeSS_noSOCE,ySS_noSOCE,~,~,~,ySSFinal_noSOCE] = SkelMuscleCa_dydt([0 1000],0, yinit, pPSO0, tic, 2, false);
pPSO(12) = pPSO(12)*ySSFinal_noSOCE(2); % set c_ref according to SS c_SR
[TimeSS_withSOCE,ySS_withSOCE,~,~,~,ySSFinal_withSOCE] = SkelMuscleCa_dydt([0 1000], 0, yinit, pPSO, tic, 1, false);
...
% pPSO(43) = 1*pSol(43)*p0(43); % PMCA
% pPSO(96) = 1.0*pSol(96)*p0(96); % total SR buffer
pPSO(12) = 0.5;%2*pSol(12)*p0(12); % set cref ratio
...
pPSO(68) = 1*pSol(68)*p0(68); % PP increased so no precipitate at SS
pPSO(70) = 1*pSol(70)*p0(70); % precipitation rate in SR
pPSO(72) = 5*pSol(72)*p0(72); % phosphate degrad
pPSO(59) = 1*pSol(59)*p0(59); % kon1 (trop binding)
pPSO(66) = 1*pSol(66)*p0(66); % hP (phosphate pushing power stroke in reverse)
pPSO(75) = 1*pSol(75)*p0(75); % myo phosphate
pPSO(74) = 1*pSol(74)*p0(74); % SR phosphate
...
for i = 1:length(tCell)
    pPSO(95) = soceVals(i);
    [Time_withSOCE,Y_withSOCE,fluxes,currents,maxCrossBridge, yinits] =...
        computeSol(pPSO, [], tSol, [3, exerciseParam], 100, phosphateAccum, geomParam, SSPhosAccum);
...
%% plot Apre->Apost rates
figure
for i = 1:length(tCell)
    forward = pPSO(65)*yCell{i}(:,crossbridgeIdx-1);
    reverse = pPSO(66)*yCell{i}(:,crossbridgeIdx).*yCell{i}(:,50)/3000;
    plot(tCell{i},forward-reverse)
    hold on
end
prettyGraph
ylabel('Powerstroke rate')
xlabel('Time (s)')


%% Write data for Nate
CaData = fluxes(:,10)*JFrac + fluxes(:,20)*BFrac;
ATPHydData = fluxes(:,9)*JFrac + fluxes(:,19)*BFrac;
dataOut = [Time_withSOCE, CaData, ATPHydData];
fileID = fopen('resistance_data.txt','w');
fprintf(fileID,'%.12f %.8f %.8f\r\n', dataOut');
fclose(fileID);

%% plot total ATP
ATPindices = [sum(juncLocLogic(1:23)), sum(juncLocLogic(:)) + sum(bulkLocLogic(1:23))];
MAindices = [sum(juncLocLogic(1:22)), sum(juncLocLogic(:)) + sum(bulkLocLogic(1:22))];
CAindices = [sum(juncLocLogic(1:16)), sum(juncLocLogic(:)) + sum(bulkLocLogic(1:16))];
ATPjunc = Y_withSOCE(:,ATPindices(1)) + Y_withSOCE(:,MAindices(1)) + Y_withSOCE(:,CAindices(1));
ATPbulk = Y_withSOCE(:,ATPindices(2)) + Y_withSOCE(:,MAindices(2)) + Y_withSOCE(:,CAindices(2));
figure
plot(Time_withSOCE, ATPjunc)
hold on
plot(Time_withSOCE, ATPbulk)
plot(Time_withSOCE, ATPjunc*JFrac + ATPbulk*BFrac)
legend('Junc ATP', 'Bulk ATP', 'Tot ATP')
